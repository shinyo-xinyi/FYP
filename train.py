...
continue_point = 40 # ocsoftmax
# continue_point = 40 # amsoftmax
# continue_point = 40 # softmax
...
def initParams():
    ...
    parser.add_argument("-f", "--path_to_features", type=str, help="features path",
                        default='features/ASVspoof2019/LA/Features_Python/')
    # parser.add_argument("--musan_path", type=str, default="dataset/others/musan",
                        # help="The path to the MUSAN dataset")
    # parser.add_argument("--rir_path", type=str, default="dataset/others/RIRS_NOISES/simulated_rirs",
    ...
    parser.add_argument('--add_loss', type=str, default="ocsoftmax", choices=["softmax", 'amsoftmax', 'ocsoftmax'],
...
def train(args):
    ...
    lfcc_optimizer = torch.optim.Adam(lfcc_model.parameters(), lr=args.lr, betas=(args.beta_1, args.beta_2), eps=args.eps, weight_decay=0.0005)

    # load the dataset
    training_set = ASVspoof2019(args.access_type, args.path_to_features, args.path_to_protocol, 'train', 'LFCC', feat_len=args.feat_len, padding=args.padding)
    validation_set = ASVspoof2019(args.access_type, args.path_to_features, args.path_to_protocol, 'dev', 'LFCC', feat_len=args.feat_len, padding=args.padding)

    # train_loader = train_loader(train_set, train_path, musan_path, rir_path, num_frames, ** kwargs)
    # validation_loader = train_loader(validation_set, train_path, musan_path, rir_path, num_frames, ** kwargs)
    ...
    # prev_eer = 1e8 # original setting

    # prev_eer = 0.0771315957864474 # softmax
    # prev_eer = 0.08869502818930006 # amsoftmax
    prev_eer = 0.0834108898175483 # ocsoftmac

    monitor_loss = args.add_loss

    for epoch_num in tqdm(range(args.num_epochs)):
        epoch_num = epoch_num + continue_point
        lfcc_model.train()
        ...
        for i, (lfcc, audio_fn, tags, labels) in enumerate(tqdm(trainDataLoader)):
            ...
            with open(os.path.join(args.out_fold, "train_loss.log"), "a") as log:
                log.write(str(epoch_num) + "\t" + str(i) + "\t" + str(np.nanmean(trainlossDict[monitor_loss])) + "\n")
