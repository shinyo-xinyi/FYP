...
def compute_eer(target_scores, nontarget_scores):
    ...
    '''
    AUC = np.trapz(frr, far)
    threshold = np.argmin(abs(frr - far))
    EER = (frr[threshold] + far[threshold]) / 2
    plt.plot(frr, far)
    plt.title('roc')
    ...
    plt.plot(frr, far, 'b-', linewidth=2, label='DET') # DET
    plt.grid(True)
    plt.legend(loc=1, fontsize='10')
    plt.ylabel('False Rejection (in %)', fontsize=10)
    plt.xlabel('False Acceptance (in %)', fontsize=10)
    ...
    '''
    # FRR和FAR随阈值的变化曲线
    plt.plot(thresholds, frr*100, 'r-', label='FRR')
    plt.plot(thresholds, far*100, 'b-', label='FAR')
    plt.legend(loc=1, fontsize='10')
    plt.ylabel('FRR/FAR (in %)', fontsize=10)
    plt.xlabel('CM Threshold', fontsize=10)
    plt.xticks(fontsize=10)
    plt.yticks(fontsize=10)
    plt.show()
    '''
