...
pathToASVspoof2019Data = 'F:\FYP\AIR-ASVspoof\dataset\LA\';
pathToFeatures = 'F:\FYP\AIR-ASVspoof\features\ASVspoof2019\LA\Features_MATLAB\';
...
% extract features for training data and store themfilePath = fullfile(pathToDatabase,['ASVspoof2019_' access_type '_train\flac'],[trainfilelist{i} '_aug.flac']);
...
%% Feature extraction for augmented training data

% extract features for training data and store themfilePath = fullfile(pathToDatabase,['ASVspoof2019_' access_type '_train\flac'],[trainfilelist{i} '_aug.flac']);
% disp('Extracting features for augmented training data...');
% trainFeatureCell = cell(length(trainfilelist), 3);
% for i=1:(length(trainfilelist)./2+1)
%     filePath = fullfile(pathToDatabase,['ASVspoof2019_' access_type '_train\flac'],[trainfilelist{i} '_aug.flac']);
%     [x,fs] = audioread(filePath);
%     [stat,delta,double_delta] = extract_lfcc(x,fs,20,512,20);
%     LFCC = [stat delta double_delta]';
%     filename_LFCC = fullfile(pathToFeatures, 'train', horzcat('LFCC_', trainfilelist{i}, '_aug.mat'))
%     parsave(filename_LFCC, LFCC)
%     LFCC = [];
% end
% disp('Done!');

%% Feature extraction for augmented development data

% extract features for augmented development data and store them
% disp('Extracting features for augmented development data...');
% for i=1:(length(devfilelist)./2+1)
%     filePath = fullfile(pathToDatabase,['ASVspoof2019_' access_type '_dev\flac'],[devfilelist{i} '_aug.flac']);
%     [x,fs] = audioread(filePath);
%     [stat,delta,double_delta] = extract_lfcc(x,fs,20,512,20);
%     LFCC = [stat delta double_delta]';
%     filename_LFCC = fullfile(pathToFeatures, 'dev', horzcat('LFCC_', devfilelist{i}, '_aug.mat'))
%     parsave(filename_LFCC, LFCC)
%     LFCC = [];
% end
% disp('Done!');

%% Feature extraction for augmented evaluation data

% extract features for augmented evaluation data and store themfilePath = fullfile(pathToDatabase,['ASVspoof2019_' access_type '_train\flac'],[trainfilelist{i} '_aug.flac']);
disp('Extracting features for augmented evaluation data...');
trainFeatureCell = cell(length(evalfilelist), 3);
for i=1:(length(evalfilelist)./2+1)
    filePath = fullfile(pathToDatabase,['ASVspoof2019_' access_type '_eval\flac'],[evalfilelist{i} '_aug.flac']);
    [x,fs] = audioread(filePath);
    [stat,delta,double_delta] = extract_lfcc(x,fs,20,512,20);
    LFCC = [stat delta double_delta]';
    filename_LFCC = fullfile(pathToFeatures, 'eval', horzcat('LFCC_', evalfilelist{i}, '_aug.mat'))
    parsave(filename_LFCC, LFCC)
    LFCC = [];
end
disp('Done!');
